...
values ('sougou_choukai_J3_001', 'sougou_choukai', 'J3', 'manual-load', '2026-09-18T08:50:50+00:00')
on conflict (id) do update set
...
values ('eabb6f222a', 0, '部下がほかの仕事で忙しいから', 'unsupported_but_plausible', '遅れの理由としてはありそうだが、会話では誰も言っていない。', null),
       ('eabb6f222a', 1, 'グラフを作るソフトが動かないから', 'correct', '部下が「ただ、グラフを作るソフトが今朝から動かなくて」と、遅れる理由として述べている。', null),
       ('eabb6f222a', 2, 'データがまだ届いていないから', 'stated_by_wrong_speaker', '課長がそう聞いただけで、部下は「昨日届きました」と否定している。', null),
       ('eabb6f222a', 3, 'システム部からの返事を待っているから', 'surface_keyword_match', 'システム部は話に出るが、返事はすでにあり、午後には直ると分かっている。', null),
       ('b2498c2196', 0, '金額が安くなった点', 'superseded_by_later_turn', '取引先が金額が変わったのかと聞いたが、営業担当は「金額は前回と同じ」と答えている。', null),
       ('b2498c2196', 1, '担当者が代わった点', 'surface_keyword_match', '「担当」という言葉は出るが、担当は変わらないと確認されている。', null),
       ('b2498c2196', 2, '数量が増えた点', 'unsupported_but_plausible', '見積の変更としてはありそうだが、数量の話は出ていない。', null),
       ('b2498c2196', 3, '納期が一週間早くなった点', 'correct', '営業担当が「納期が一週間早くなりました」と、変わった点として述べている。', null)
