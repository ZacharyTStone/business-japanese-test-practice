...
values ('f0575e4dfd', 0, '木曜日までに、コメントをメールで佐藤に送る。', 'surface_keyword_match', 'コメントと期限は合っているが、注記は「メールでは送らない」と書いている。', null),
       ...
       ('e6cfbc7831', 1, '二十日までには三十個しか納品できず、残りは二十五日以降になること', 'correct', '在庫は三十個で追加の入荷は二十五日なのに、先方は五十個を二十日までと頼んでいる。二つのメールを合わせると、注文どおりには納品できない。', null),
       ('e6cfbc7831', 2, '注文の数量が五十個であること', 'stated_but_answers_different_question', 'メールに書かれている事実だが、それ自体は問題ではない。', null),
       ('e6cfbc7831', 3, '追加の入荷が二十五日より遅れること', 'wrong_timeframe', '入荷は二十五日の予定と書かれているだけで、遅れるとは書かれていない。', null)
