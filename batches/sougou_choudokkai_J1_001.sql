...
values ('91d768f9f8', 0, '10月7日ごろまで', 'unsupported_but_plausible', '検収の遅れに合わせて完成も1週間遅らせた形だが、納品日を動かさないと約束している以上、遅らせる余地はない。', null),
       ('91d768f9f8', 1, '10月15日まで', 'stated_by_wrong_speaker', '部下が最初に挙げた日付だが、課長がそれは先方の希望する納品日であって完成の期限ではないと述べている。', null),
       ('91d768f9f8', 2, '9月23日ごろまで', 'correct', '報告書の完成目安は9月30日だが、検収の開始が1週間遅れるのに納品日を動かさないので、完成もその分、1週間早める必要がある。', null),
       ('91d768f9f8', 3, '9月30日まで', 'combines_wrong_pair', '報告書に書かれた目安のままで、検収が1週間遅れて始まるという課長の話を反映していない。', null),
       ('749cfab0e3', 0, '納期を1か月延ばして設置費を減らし、予算を超える分は初年度の保守の値引きで補う。', 'correct', '課長が案二を前提に「超える分は初年度の保守を値引きで吸収」「台数はそのまま」と決めており、案二に値引きを足した形になる。', null),
       ('749cfab0e3', 1, '台数を16台に減らして、予算内に収める。', 'combines_wrong_pair', '表の中で予算内に収まる案だが、課長が来年の追加でかえって高くつくと退けている。', null),
       ('749cfab0e3', 2, '保守契約を外して、最も安い案にする。', 'stated_by_wrong_speaker', '部下が用意した案の一つではあるが、先方が保守を重視していると課長が指摘し、部下も実質外れると認めている。', null),
       ('749cfab0e3', 3, '納期を1か月延ばして設置費を減らし、超える分は先方に予算を増やしてもらう。', 'wrong_action_owner', '案二を通す方法としてはあり得るが、超過分を吸収するのは自社の保守の値引きで、先方に予算を動かしてもらう話は出ていない。', null)
