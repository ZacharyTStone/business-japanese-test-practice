...
const S = {
  ...
  // choose
  choose_title: ["自分で選ぶ", "Choose yourself"],
  choose_sub: ["ふだんは「今日の練習」で十分です。種類をしぼりたいときに。いまのレベル {level}", "Most days “Practice today” is enough. Use this to focus on one type. Level {level}"],
  coming_soon: ["近日公開", "Coming soon"],
  no_items_title: ["いまのレベルの問題はまだありません", "No questions at your level yet"],
  no_items_body: ["公開されるのをお待ちください。「今日の練習」は隣のレベルからも出します。", "Please wait for them to be published. “Practice today” also draws from the neighbouring levels."],
  btn_type: ["この種類で練習する", "Practice this type"],
  btn_type_sub: ["種類を選ばない場合は、すべての種類から出します", "With no type selected, questions come from all types"],
  btn_mock: ["模試をやる", "Take a mock exam"],
  btn_mock_sub: ["20問・途中でやめずに最後まで", "20 questions · finish in one sitting"],
  mock_note: ["模試でも点数の予測は出しません。生成した問題に対する換算の根拠がないからです。", "The mock doesn't predict a score: there is no basis for converting generated questions into one."],
  ...
  scene_hint_mock: ["本番と同じく、一回だけ流れます。聞き直せません。", "As in the exam: it plays once. No replay."],
  ...
  mode_mock: ["模試", "Mock exam"],
  mode_this: ["今回", "This set"],
  n_correct: ["{n}問 正解", "{n} correct"],
  n_min: ["{total}問・{min}分", "{total} questions · {min} min"],
  ring_correct: ["正解", "right"],
  mock_noscore_title: ["点数の予測は出していません", "No score prediction"],
  mock_noscore_body: ["ここにある問題は本番の問題ではなく、何点に当たるかを換算する根拠がありません。正答数と、下に出ている「ミス」のほうが、次に何をすればよいかをはっきり示します。", "These are not real exam questions, so there is no basis for converting them into points. The count and the mistakes below say more clearly what to do next."],
  ...
  once_only: ["音声は一回だけでした（模試）", "Audio played once (mock exam)"],
